# Zero CLI Local Agent Playbook

This playbook is a practical guide to run Zero CLI with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Zero CLI starts directly.
...
Or let Zero CLI recommend the best local model for your goal:
...
```powershell
Get-Content .\.zerocli-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\zero\zero
bun run dev:profile
```
...
- Keep `.zerocli-profile.json` local (already gitignored).
