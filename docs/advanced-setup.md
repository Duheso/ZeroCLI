...
If you use the in-app provider wizard, choose `Codex OAuth` to open ChatGPT sign-in in your browser and let Zero CLI store Codex credentials securely.

If you already use the Codex CLI, Zero CLI reads `~/.codex/auth.json` automatically. You can also point it elsewhere with `CODEX_AUTH_JSON_PATH` or override the token directly with `CODEX_API_KEY`.
...
```bash
ollama launch zero --model llama3.3:70b
```
...
| `ZERO_DISABLE_CO_AUTHORED_BY` | No | Suppress the default `Co-Authored-By` trailer in generated git commits |
...
# launch using persisted profile (.zerocli-profile.json)
bun run dev:profile
