# Zero CLI on Android (Termux)

A complete guide to running Zero CLI on Android using Termux + proot Ubuntu.
...
Zero CLI requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone Zero CLI

```bash
git clone https://github.com/Duheso/ZeroCLI.git
cd zero
npm install
npm link
```
...
### Step 6 — Build Zero CLI

```bash
cd /data/data/com.termux/files/home/zero
bun run build
```

You should see:
```
✓ Built zero v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to Zero CLI's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Zero CLI repo.
