...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('zerocli');
  const launchCommand = configured.get('launchCommand', 'zero');
  const terminalName = configured.get('terminalName', 'Zero CLI');
...
async function launchZeroCLI(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `Zero CLI command not found: ${executable}. Install it with: npm install -g @duheso/zerocli`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(ZERO_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(ZERO_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch Zero CLI because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="Zero CLI wordmark">Zero <span class="wordmark-accent">CLI</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream Zero CLI project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and Zero CLI commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class ZeroCLIControlCenterProvider {
  constructor() {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          ...
          break;
        case 'launchRoot':
          await launchZeroCLI({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const chatViewProvider = new ZeroCLIChatViewProvider(chatController);
  const chatPanelManager = new ZeroCLIChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) Zero CLI';
  statusBarItem.tooltip = 'Open Zero CLI Chat';
  statusBarItem.command = 'zerocli.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) Zero CLI';
        statusBarItem.tooltip = 'Zero CLI is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) Zero CLI';
        statusBarItem.tooltip = 'Zero CLI connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) Zero CLI';
        statusBarItem.tooltip = 'Open Zero CLI Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('zerocli.start', async () => {
    await launchZeroCLI();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'zerocli.startInWorkspaceRoot',
    async () => {
      await launchZeroCLI({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('zerocli.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(ZERO_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'zerocli.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(ZERO_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'zerocli.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('zerocli.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.zero');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('zerocli.newChat', () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('zerocli.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'zerocli.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('zero')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  ZeroCLIControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  ZeroCLIChatViewProvider,
  ZeroCLIChatPanelManager,
};
