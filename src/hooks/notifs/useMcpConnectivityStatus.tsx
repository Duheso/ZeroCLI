...
import { hasZeroAiMcpEverConnected } from '../../services/mcp/claudeai.js';
  ...
  if ($[0] !== addNotification || $[1] !== mcpClients) {
    t2 = () => {
      try {
        ...
        const failedZeroAiClients = mcpClients.filter(_temp2);
        const needsAuthLocalServers = mcpClients.filter(_temp3);
        const needsAuthZeroAiServers = mcpClients.filter(_temp4);
        if (failedLocalClients.length === 0 && failedZeroAiClients.length === 0 && needsAuthLocalServers.length === 0 && needsAuthZeroAiServers.length === 0) {
          return;
        }
        ...
        if (failedZeroAiClients.length > 0) {
          addNotification({
            key: "mcp-claudeai-failed",
            jsx: <><Text color="error">{failedZeroAiClients.length} claude.ai{" "}{failedZeroAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
        ...
        if (needsAuthZeroAiServers.length > 0) {
          addNotification({
            key: "mcp-claudeai-needs-auth",
            jsx: <><Text color="warning">{needsAuthZeroAiServers.length} claude.ai{" "}{needsAuthZeroAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
      } catch (error) {
...
function _temp4(client_2: MCPServerConnection) {
  return client_2.type === "needs-auth" && client_2.config.type === "claudeai-proxy" && hasZeroAiMcpEverConnected(client_2.name);
}
...
function _temp2(client_0: MCPServerConnection) {
  return client_0.type === "failed" && client_0.config.type === "claudeai-proxy" && hasZeroAiMcpEverConnected(client_0.name);
}
