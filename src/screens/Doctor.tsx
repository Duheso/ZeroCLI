...
import { getZeroConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0: {
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getZeroConfigHomeDir(), "agents");
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = join(getXDGStateHome(), "zerocli", "locks");
