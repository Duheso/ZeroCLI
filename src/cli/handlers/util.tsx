    ...
    root.render(<AppStateProvider onChangeAppState={onChangeAppState}>
        <KeybindingSetup>
          <Box flexDirection="column" gap={1}>
            ...
            void resolve();
          }} mode="setup-token" startingMessage="This will guide you through long-lived (1-year) auth token setup for your Zero account. Zero subscription required." />
          </Box>
        </KeybindingSetup>
      </AppStateProvider>);
  });
...
function DoctorWithPlugins(t0: { onDone: (result?: string, options?: { display?: unknown }) => void }) {
  const $ = _c(2);
  const {
    onDone
  } = t0;
