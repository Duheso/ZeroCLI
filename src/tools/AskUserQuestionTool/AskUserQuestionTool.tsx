...
function AskUserQuestionResultMessage(t0: { answers: Record<string, string> }) {
  const $ = _c(3);
  const {
    answers
  } = t0;
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t1 = <Box flexDirection="row"><Text color={getModeColor("default")}>{BLACK_CIRCLE} </Text><Text>User answered Zero's questions:</Text></Box>;
    $[0] = t1;
  } else {
    t1 = $[0];
  }
