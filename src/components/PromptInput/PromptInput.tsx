...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(s => ("external" === 'ant' as unknown) as boolean && s.tungstenActiveSession !== undefined);
  const tmuxFooterVisible = ("external" === 'ant' as unknown) as boolean && hasTungstenSession;
  ...
  const hasBgTaskPill = useMemo(() => Object.values(tasks).some(t => isBackgroundTask(t) && !(("external" === 'ant' as unknown) as boolean && isPanelAgentTask(t))), [tasks]);
  ...
  const tasksFooterVisible = (runningTaskCount > 0 || (("external" === 'ant' as unknown) as boolean && coordinatorTaskCount > 0)) && !shouldHideTasksFooter(tasks, showSpinnerTree);
  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in ZeroCLI on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  useEffect(() => {
    if (isUltrareviewEnabled() && ultrareviewTriggers.length) {
      addNotification({
        ...
        text: 'Run /ultrareview after Zero finishes to review these changes in the cloud',
        priority: 'immediate',
        timeoutMs: 5000
      });
    }
  ...
  useKeybindings({
    'footer:up': () => {
      ...
      if (tasksSelected && ("external" === 'ant' as unknown) as boolean && coordinatorTaskCount > 0 && coordinatorTaskIndex > minCoordinatorIndex) {
    ...
    'footer:down': () => {
      ...
      if (tasksSelected && ("external" === 'ant' as unknown) as boolean && coordinatorTaskCount > 0) {
    ...
    'footer:openSelected': () => {
      ...
      switch (footerItemSelected) {
        ...
        case 'tasks':
          ...
          break;
        case 'tmux':
          if (("external" === 'ant' as unknown) as boolean) {
