...
export function Tabs(t0: TabsProps) {
  ...
  const tabs = children.map(_temp) as [string, string][];
  const defaultTabIndex = defaultTab ? tabs.findIndex((tab: [string, string]) => defaultTab === tab[0]) : 0;
  ...
  const controlledTabIndex = isControlled ? tabs.findIndex((tab_0: [string, string]) => tab_0[0] === controlledSelectedTab) : -1;
  ...
  const t15 = !hidden && <Box key={`${selectedTabIndex}-${headerFocused ? "focused" : "blurred"}`} flexDirection="row" gap={1} flexShrink={modalScrollRef ? 0 : undefined}>{title !== undefined && <Text bold={true} color={color}>{title}</Text>}{tabs.map((t16: [string, string], i: number) => {
...
function _temp(child: React.ReactElement<TabProps>) {
  return [child.props.id ?? child.props.title, child.props.title] as [string, string];
}
type TabProps = {
  title: string;
  id?: string;
