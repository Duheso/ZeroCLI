...
function getToolBuckets(): ToolBuckets {
  return {
    ...
    EXECUTION: {
      ...
      toolNames: new Set([BashTool.name, (("external" as string) === 'ant' ? (TungstenTool as unknown as Tool)?.name : undefined) as string | undefined].filter((n: string | undefined) => n !== undefined))
    },
    MCP: {
      name: 'MCP tools',
      toolNames: new Set(),
...
export function ToolSelector(t0: Props) {
  ...
  if ($[59] !== focusIndex || $[60] !== t18) {
    t19 = t18.map((item_0: { id: string; label: string; action: () => void; isContinue?: boolean; isToggle?: boolean; isHeader?: boolean }, index: number) => {
