...
export function AgentsList(t0: Props) {
  ...
  if ($[2] !== isCreateNewSelected) {
    t2 = () => <Box><Text color={isCreateNewSelected ? "suggestion" : undefined}>{isCreateNewSelected ? `${figures.pointer} ` : "  "}</Text><Text color={isCreateNewSelected ? "suggestion" : undefined}>{t('agents_create_new')}</Text></Box>;
  ...
  if ($[4] !== isCreateNewSelected || $[5] !== selectedAgent?.agentType || $[6] !== selectedAgent?.source) {
    t3 = (agent_0: ResolvedAgent) => {
      ...
      return <Box key={`${agent_0.agentType}-${agent_0.source}`}><Text dimColor={dimmed && !isSelected} color={textColor}>{isBuiltIn ? "" : isSelected ? `${figures.pointer} ` : "  "}</Text><Text dimColor={dimmed && !isSelected} color={textColor}>{agent_0.agentType}</Text>{resolvedModel && <Text dimColor={true} color={textColor}>{" \xB7 "}{resolvedModel}</Text>}{agent_0.memory && <Text dimColor={true} color={textColor}>{" \xB7 "}{agent_0.memory}{t('agents_memory_suffix')}</Text>}{overriddenBy && <Text dimColor={!isSelected} color={isSelected ? "warning" : undefined}>{" "}{figures.warning}{t('agents_shadowed_by')}{getOverrideSourceLabel(overriddenBy)}</Text>}</Box>;
    };
    ...
    bb1: {
      ...
      if (hasNoAgents) {
        ...
        if ($[58] === Symbol.for("react.memo_cache_sentinel")) {
          t24 = <Text dimColor={true}>{t('agents_no_agents_desc1')}</Text>;
          t25 = <Text dimColor={true}>{t('agents_no_agents_desc2')}</Text>;
          t26 = <Text dimColor={true}>{t('agents_no_agents_desc3')}</Text>;
          $[58] = t24;
          $[59] = t25;
          $[60] = t26;
        } else {
        ...
        if ($[69] !== onBack || $[70] !== sourceTitle || $[71] !== t28) {
          t29 = <Dialog title={sourceTitle} subtitle={t('agents_no_agents_subtitle')} onCancel={onBack} hideInputGuide={true}>{t28}</Dialog>;
          $[69] = onBack;
          $[70] = sourceTitle;
          $[71] = t28;
          $[72] = t29;
