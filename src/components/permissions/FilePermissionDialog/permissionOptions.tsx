 ...
 */
export function isInZeroFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath);
  const claudeFolderPath = expandPath(`${getOriginalCwd()}/.claude`);

  // Check if the path is within the project's .claude folder
  const normalizedAbsolutePath = normalizeCaseForComparison(absolutePath);
  const normalizedZeroFolderPath = normalizeCaseForComparison(claudeFolderPath);

  // Path must start with the .claude folder path (and be inside it, not just the folder itself)
  return normalizedAbsolutePath.startsWith(normalizedZeroFolderPath + sep.toLowerCase()) ||
  // Also match case where sep is / on posix systems
  normalizedAbsolutePath.startsWith(normalizedZeroFolderPath + '/');
}
 ...
 */
export function isInGlobalZeroFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath);
  const globalZeroFolderPath = join(homedir(), '.claude');
  const normalizedAbsolutePath = normalizeCaseForComparison(absolutePath);
  const normalizedGlobalZeroFolderPath = normalizeCaseForComparison(globalZeroFolderPath);
  return normalizedAbsolutePath.startsWith(normalizedGlobalZeroFolderPath + sep.toLowerCase()) || normalizedAbsolutePath.startsWith(normalizedGlobalZeroFolderPath + '/');
}
...
export function getFilePermissionOptions({
  ...
  if (yesInputMode && onAcceptFeedbackChange) {
    options.push({
      ...
      label: 'Yes',
      value: 'yes',
      placeholder: 'and tell Zero what to do next',
  ...
  const inZeroFolder = isInZeroFolder(filePath);
  const inGlobalZeroFolder = isInGlobalZeroFolder(filePath);

  // Option 2: For .claude/ folder, show special option instead of generic session option
  // Note: Session-level options are always shown since they only affect in-memory state,
  // not persisted settings. The allowManagedPermissionRulesOnly setting only restricts
  // persisted permission rules.
  if ((inZeroFolder || inGlobalZeroFolder) && operationType !== 'read') {
    options.push({
      label: 'Yes, and allow Zero to edit its own settings for this session',
      value: 'yes-claude-folder',
      option: {
        type: 'accept-session',
        scope: inGlobalZeroFolder ? 'global-claude-folder' : 'claude-folder'
      }
    });
  } else {
  ...
  if (noInputMode && onRejectFeedbackChange) {
    options.push({
      ...
      label: 'No',
      value: 'no',
      placeholder: 'and tell Zero what to do differently',
