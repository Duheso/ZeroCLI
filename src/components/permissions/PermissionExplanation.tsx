...
/**
 * Setter updater for toggling visibility.
 */
...
function ExplanationResult(t0: { promise: Promise<PermissionExplanationType | null> }) {
  ...
  const explanation = use(promise) as PermissionExplanationType;
  if (!explanation) {
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
 ...
 */
export function PermissionExplainerContent(t0: { visible: boolean, promise: Promise<PermissionExplanationType | null> }) {
  const $ = _c(3);
  const {
    visible,
    promise
