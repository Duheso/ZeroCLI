...
export function SandboxPermissionRequest(t0: SandboxPermissionRequestProps) {
  const $ = _c(22);
  const {
    hostPattern: t1,
    onUserResponse
  ...
  let t2;
  if ($[0] !== onUserResponse) {
    t2 = function onSelect(value: string) {
      bb4: switch (value) {
        case "yes":
          {
            onUserResponse({
  ...
  if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = {
      label: <Text>No, and tell Zero what to do differently <Text bold={true}>(esc)</Text></Text>,
      value: "no"
    };
    $[6] = t6;
  } else {
