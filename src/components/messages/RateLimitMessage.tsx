...
import { getRateLimitTier, getSubscriptionType, isZeroAISubscriber } from 'src/utils/auth.js';
...
export function RateLimitMessage(t0: any) {
  const $ = _c(16);
  const {
    text,
  ...
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = shouldProcessMockLimits() || isZeroAISubscriber();
    $[2] = t3;
  } else {
    t3 = $[2];
  }
  ...
  bb0: {
    ...
    if ($[7] !== shouldAutoOpenRateLimitOptionsMenu) {
      t7 = getUpsellMessage({
        ...
        isTeamOrEnterprise,
        hasBillingAccess: hasZeroAiBillingAccess()
      });
