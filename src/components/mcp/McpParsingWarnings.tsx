...
function McpConfigErrorSection(t0: { scope: ConfigScope; parsingErrors: ValidationError[]; warnings: ValidationError[] }) {
  const $ = _c(26);
  const {
    scope,
    parsingErrors,
...
function _temp5(t0: { scope: ConfigScope; config: { errors: ValidationError[] } }) {
  const {
    scope,
    config: config_1
  } = t0;
...
function _temp4(t0: { config: { errors: ValidationError[] } }) {
  const {
    config: config_0
  } = t0;
  return filterErrors(config_0.errors, "warning").length > 0;
}
function _temp3(t0: { config: { errors: ValidationError[] } }) {
  const {
    config
  } = t0;
