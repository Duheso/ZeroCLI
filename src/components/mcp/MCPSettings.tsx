...
import { ZeroAuthProvider } from '../../services/mcp/auth.js';
    ...
    t5 = () => {
      ...
      const prepareServers = async function prepareServers() {
        const serverInfos = await Promise.all(filteredClients.map(async (client_0: MCPServerConnection) => {
          ...
          const isZeroAIProxy = client_0.config.type === "claudeai-proxy";
          let isAuthenticated = undefined;
          if (isSSE || isHTTP) {
            const authProvider = new ZeroAuthProvider(client_0.name, client_0.config as McpSSEServerConfig | McpHTTPServerConfig);
          ...
          if (isZeroAIProxy) {
            return {
              ...
              config: client_0.config as McpZeroAIProxyServerConfig
            };
          } else {
            if (isSSE) {
              return {
    ...
    case "server-menu":
      {
        ...
        const defaultTab = viewState.server.transport === "claudeai-proxy" ? "claude.ai" : "ZeroCLI";
