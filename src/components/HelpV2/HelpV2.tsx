...
export function HelpV2(t0: Props) {
  ...
  if ($[3] !== commands) {
    ...
    builtinCommands = commands.filter((cmd: Command) => builtinNames.has(cmd.name) && !cmd.isHidden);
    let t4: Command[] = [];
  ...
  let t6;
  if ($[31] !== tabs) {
    t6 = <Tabs title={false ? "/help" : `ZeroCLI v${MACRO.VERSION}`} color="professionalBlue" defaultTab="general">{tabs}</Tabs>;
    $[31] = tabs;
    $[32] = t6;
  } else {
    t6 = $[32];
