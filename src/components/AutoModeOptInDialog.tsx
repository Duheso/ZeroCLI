...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Zero handle permission prompts automatically — Zero checks each tool call for risky actions and prompt injection before executing. Actions Zero identifies as safe are executed, while actions Zero identifies as risky are blocked and Zero may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Zero can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
