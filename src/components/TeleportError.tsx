...
import { checkIsGitClean, checkNeedsZeroAiLogin } from 'src/utils/background/remote/preconditions.js';
...
export function TeleportError(t0: TeleportErrorProps) {
  ...
  const handleLoginWithZeroAI = t6;
  let t7;
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = (value: string) => {
      if (value === "login") {
        handleLoginWithZeroAI();
      } else {
        onCancel();
      }
    };
    ...
    case "needsLogin":
      {
        ...
        if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
          t9 = <Box flexDirection="column"><Text dimColor={true}>Teleport requires a Zero.ai account.</Text><Text dimColor={true}>Your Zero Pro/Max subscription will be used by ZeroCLI.</Text></Box>;
          $[16] = t9;
        } else {
          t9 = $[16];
        }
        ...
        if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
          t10 = <Dialog title="Log in to Zero" onCancel={onCancel}>{t9}<Select options={[{
              label: "Login with Zero account",
              value: "login"
            }, {
              label: "Exit",
              value: "exit"
...
export async function getTeleportErrors(): Promise<Set<TeleportLocalErrorType>> {
  ...
  const [needsLogin, isGitClean] = await Promise.all([checkNeedsZeroAiLogin(), checkIsGitClean()]);
  if (needsLogin) {
