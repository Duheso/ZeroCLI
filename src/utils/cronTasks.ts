...
const CRON_FILE_REL = join('.zerocli', 'scheduled_tasks.json')
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.zerocli'), { recursive: true })
 ...
 * and recurring tasks — a recurring task whose window passed while Zero
