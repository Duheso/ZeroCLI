/**
 * ZeroCLI hints protocol.
 *
 * CLIs and SDKs running under ZeroCLI can emit a self-closing
 ...
 */
export function extractZeroCodeHints(
  output: string,
  command: string,
): { hints: ZeroCodeHint[]; stripped: string } {
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    hints.push({ v, type: type as ZeroCodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: ZeroCodeHint | null = null
...
export function setPendingHint(hint: ZeroCodeHint): void {
...
export function getPendingHintSnapshot(): ZeroCodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetZeroCodeHintStore(): void {
  pendingHint = null
