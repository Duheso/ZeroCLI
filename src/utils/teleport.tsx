...
import { checkAndRefreshOAuthTokenIfNeeded, getZeroAIOAuthTokens } from './auth.js';
...
/**
 * Generates a title and branch name for a coding session using Zero Haiku
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getZeroAIOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('ZeroCLI web sessions require authentication with a Zero.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
...
/**
 * Creates a remote Zero.ai session with error handling and UI feedback.
...
/**
 * Creates a remote Zero.ai session using the Sessions API.
 *
 * Two source modes:
