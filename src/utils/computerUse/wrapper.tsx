...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another Zero session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'Zero is using your computer · press Esc to stop' : 'Zero is using your computer · press Ctrl+C to stop',
