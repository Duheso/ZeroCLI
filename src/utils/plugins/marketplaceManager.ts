/**
 * Marketplace manager for ZeroCLI plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a ZeroCLI bug, shouldn't hit the error file.
    return {}
  }
}
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Zero-Code-Plugin-Manager',
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `ZeroCLI version. Run 'claude marketplace remove ${name}' and ` +
