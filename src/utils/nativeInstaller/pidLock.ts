 ...
 *
 * This module provides PID-based locking for running ZeroCLI versions.
...
/**
 * Validate that a running process is actually a Zero process
 * This helps mitigate PID reuse issues
 */
function isZeroProcess(pid: number, expectedExecPath: string): boolean {
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a Zero process?
  // This helps with PID reuse scenarios
  if (!isZeroProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be Zero - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a Zero process (to handle PID reuse scenarios)
