...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Zero
  ...
  function check() {
    ...
    // exists to stop two Zero sessions in the same cwd from double-firing
  ...
  async function enable() {
    ...
    // dies. Prevents double-firing when multiple Zeros share a cwd.
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while Zero was not running. ` +
