...
/**
 * Generates ZeroCLI specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getZeroCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Zero's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and ZeroCLI content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const claudeCodeContent = await getZeroCodeSnapshotContent()
 ...
 *
 * This function is a critical part of Zero CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Zero CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with ZeroCLI defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getZeroConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Zero home: ${getZeroConfigHomeDir()}`)
