...
import { getZeroAIOAuthTokens } from '../auth.js'
...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'ZeroCLI web sessions require authentication with a Zero.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
