 ...
 * =====================
 * This module manages an isolated tmux socket for Zero's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Zero could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Zero from within tmux.
 *
 * HOW IT WORKS:
 * 1. Zero creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *    (set in Shell.ts via getZeroTmuxEnv())
 *
 * This means ANY tmux command run through Zero - whether via the Tmux tool
 * directly or via Bash - will operate on Zero's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getZeroTmuxEnv() returns a value that overrides the
...
const CLAUDE_SOCKET_PREFIX = 'zero'
...
/**
 * Gets the socket name for Zero's isolated tmux session.
 * Format: claude-<PID>
 */
export function getZeroSocketName(): string {
  if (!socketName) {
 ...
 */
export function getZeroSocketPath(): string | null {
  return socketPath
}
 ...
 */
export function setZeroSocketInfo(path: string, pid: number): void {
  socketPath = path
  serverPid = pid
}
...
/**
 * Gets the TMUX environment variable value for Zero's isolated socket.
 *
 ...
 * the Bash tool will operate on Zero's socket, NOT the user's session.
 *
 ...
 */
export function getZeroTmuxEnv(): string | null {
 ...
 * initializing the socket. getZeroTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Any ZeroCLI instance spawned on this socket will inherit this env var,
  ...
  if (pidResult.code === 0) {
    ...
    if (!isNaN(pid)) {
      ...
      setZeroSocketInfo(fallbackPath, pid)
      return
    }
    // PID parsing failed
    logForDebugging(
