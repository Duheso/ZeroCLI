...
test('install command displays ~/.local/bin/zero on non-Windows', async () => {
  ...
  expect(getInstallationPath()).toBe('~/.local/bin/zero')
})

test('install command displays zero.exe path on Windows', async () => {
  mock.module('../utils/env.js', () => ({
    env: { platform: 'win32' },
  }))
  ...
  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'zero.exe').replace(/\//g, '\\'),
  )
})
...
test('cleanupNpmInstallations removes both zero and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    getZeroConfigHomeDir: () => join(homedir(), '.zerocli'),
