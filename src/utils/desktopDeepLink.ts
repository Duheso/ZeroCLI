...
/**
 * Check if Zero Desktop app is installed.
 * On macOS, checks for /Applications/Zero.app.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Zero.app in /Applications
    return pathExists('/Applications/Zero.app')
...
/**
 * Detect the installed Zero Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/Zero.app/Contents/Info.plist',
    ...
    const installDir = join(localAppData, 'AnthropicZero')
    try {
...
/**
 * Build and open a deep link to resume the current session in Zero Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Zero Desktop is not installed. Install it from https://claude.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Zero Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
