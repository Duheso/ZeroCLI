...
import type { AssistantMessage, AttachmentMessage, Message, NormalizedUserMessage, ProgressMessage, SystemMessage, UserMessage } from 'src/types/message.js';
...
async function executeForkedSlashCommand(command: CommandBase & PromptCommand, args: string, context: ProcessUserInputContext, precedingInputBlocks: ContentBlockParam[], setToolJSX: SetToolJSXFn, canUseTool: CanUseToolFn): Promise<SlashCommandResult> {
  ...
  const createProgressMessage = (message: AssistantMessage | NormalizedUserMessage): ProgressMessage<AgentProgress> => {
    ...
    return {
      type: 'progress',
      data: {
        message: message as NormalizedUserMessage,
        type: 'agent_progress',
        prompt: skillContent,
        agentId
      },
  ...
  if (command.userInvocable === false) {
    return {
      messages: [createUserMessage({
        ...
        content: `This skill can only be invoked by Zero, not directly by users. Ask Zero to use the "${commandName}" skill for you.`
      })],
      shouldQuery: false,
      command
    };
      ...
      case 'local':
        {
          ...
          try {
            ...
            if (result.type === 'compact') {
              ...
              return {
                messages: buildPostCompactMessages(compactionResultWithSlashMessages) as (UserMessage | AssistantMessage | SystemMessage | AttachmentMessage | ProgressMessage)[],
                shouldQuery: false,
                command
              };
            }
