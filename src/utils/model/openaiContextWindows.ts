...
const OPENAI_CONTEXT_WINDOWS: Record<string, number> = {
  ...
  // LiteLLM format — when Zero CLI talks to a LiteLLM proxy, Copilot models
  ...
  // NOTE: bare Zero model names (e.g. 'claude-sonnet-4') are intentionally
...
const OPENAI_MAX_OUTPUT_TOKENS: Record<string, number> = {
  ...
  // NOTE: bare Zero model names omitted — see context windows comment above.
