...
export async function getZeroDesktopConfigPath(): Promise<string> {
  const platform = getPlatform()

  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Zero Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/Zero/claude_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for Zero Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Zero Desktop config file in Windows. Make sure Zero Desktop is installed on Windows.',
  )
}

export async function readZeroDesktopMcpServers(): Promise<
  Record<string, McpServerConfig>
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Zero Desktop integration only works on macOS and WSL.',
    )
  }
  try {
    const configPath = await getZeroDesktopConfigPath()
