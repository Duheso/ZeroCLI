...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: zero completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const claudeBin = process.argv[1] || 'zero'
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: zero completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('zero completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.zerocli/.
 * Called after `zero update` so completions stay in sync with the new binary.
 */
