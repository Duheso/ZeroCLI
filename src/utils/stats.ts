...
/**
 * Convert a PersistedStatsCache to ZeroCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all ZeroCLI sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateZeroCodeStats(): Promise<ZeroCodeStats> {
 ...
 */
export async function aggregateZeroCodeStatsForRange(
  range: StatsDateRange,
): Promise<ZeroCodeStats> {
  if (range === 'all') {
    return aggregateZeroCodeStats()
  }
  ...
  return processedStatsToZeroCodeStats(stats)
}

/**
 * Convert ProcessedStats to ZeroCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToZeroCodeStats(
...
function getEmptyStats(): ZeroCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
