...
/**
 * Attribution state for tracking Zero's contributions to files.
 */
...
/**
 * Summary of Zero's contribution for a commit.
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate Zero's character contribution
...
/**
 * Track a file modification by Zero.
...
/**
 * Track a file creation by Zero (e.g., via bash command).
 * Used when Zero creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by Zero (e.g., via bash rm command).
 * Used when Zero deletes a file through a non-tracked mechanism.
 */
...
export async function calculateCommitAttribution(
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // Zero deleted this file (tracked deletion)
  ...
  for (const result of fileResults) {
    ...
    totalZeroChars += result.claudeChars
  ...
  const totalChars = totalZeroChars + totalHumanChars
  const claudePercent =
    totalChars > 0 ? Math.round((totalZeroChars / totalChars) * 100) : 0
