...
export const CLAUDE_IN_CHROME_MCP_SERVER_NAME = 'zero-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Zero in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isZeroInChromeMCPServer(name: string): boolean {
...
export function trackZeroInChromeTabId(tabId: number): void {
...
export function isTrackedZeroInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Zero in Chrome] No compatible browser found')
    return false
  }
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- ZeroForChromeContext.getSocketPaths (external @ant/claude-for-chrome-mcp) requires a sync () => string[] callback
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Zero in Chrome MCP
