...
import { trackZeroInChromeTabId } from './common.js';
...
/**
 * Renders a clickable "View Tab" link for Zero in Chrome MCP tools.
 * Returns null if:
 * - The tool is not a Zero in Chrome MCP tool
...
/**
 * Custom tool result message rendering for zero-in-chrome tools.
...
/**
 * Returns tool method overrides for Zero in Chrome MCP tools. Use this to customize
 * rendering for chrome tools in a single spread operation.
 */
export function getZeroInChromeMCPToolOverrides(toolName: string): {
  ...
  return {
    userFacingName(_input?: Record<string, unknown>) {
      ...
      return `Zero in Chrome[${displayName}]`;
    },
