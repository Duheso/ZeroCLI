...
const APP_NAME = 'ZeroCLI URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'ZeroCLI URL Handler.app'
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>ZeroCLI Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for ZeroCLI
...
/**
 * Resolve the zero binary path for protocol registration. Prefers the
 * native installer's stable symlink (~/.local/bin/zero) which survives
...
async function resolveZeroPath(): Promise<string> {
  const binaryName = process.platform === 'win32' ? 'zero.exe' : 'zero'
 ...
 * a cached flag in ~/.zerocli.json, so:
