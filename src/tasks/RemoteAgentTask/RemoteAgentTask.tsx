...
type SDKAssistantMessage = SDKMessage & {
  type: 'assistant';
  message: { content: ReadonlyArray<{ type: string; [key: string]: unknown }> };
};
...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your Zero.ai account (not Console).';
    ...
    case 'github_app_not_installed':
      return 'The Zero GitHub app must be installed on this repository first.\nhttps://github.com/apps/claude/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Zero never
...
function startRemoteSessionPolling(taskId: string, context: TaskContext): () => void {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (logGrew) {
        ...
        const deltaText = response.newEvents.map(msg => {
          if (msg.type === 'assistant') {
            return (msg.message as { content: readonly { type: string; text?: string }[] }).content.filter(block => block.type === 'text').map(block => 'text' in block ? block.text : '').join('\n');
          }
      ...
      // stableIdle is a prompt-mode completion signal (Zero stops writing
...
/**
 * RemoteAgentTask - Handles remote Zero.ai session execution.
 *
