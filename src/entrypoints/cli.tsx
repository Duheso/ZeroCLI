...
// Zero CLI: polyfill globalThis.File for Node < 20.
...
// Zero CLI: disable experimental API betas by default.
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (ZeroCLI)`);
    return;
  }
  ...
  if (process.argv[2] === '--zero-in-chrome-mcp') {
    ...
    await runZeroInChromeMcpServer();
    return;
  ...
  if (process.env.ZERO_DISABLE_EARLY_INPUT !== '1') {
    const {
