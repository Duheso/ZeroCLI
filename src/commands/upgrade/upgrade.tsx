...
import { getZeroAIOAuthTokens, isZeroAISubscriber } from '../../utils/auth.js';
  ...
  try {
    ...
    if (isZeroAISubscriber()) {
      ...
      if (tokens?.subscriptionType && tokens?.rateLimitTier) {
        ...
        isMax20x = (profile?.organization as any)?.organization_type === 'claude_max' && (profile?.organization as any)?.rate_limit_tier === 'default_claude_max_20x';
      }
      if (isMax20x) {
