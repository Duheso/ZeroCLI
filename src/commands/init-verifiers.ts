...
const command = {
  type: 'prompt',
  name: 'init-verifiers',
  description: t('cmd_init_verifiers') as string,
...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Zero Chrome Extension MCP (browser-use via Zero's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Zero Chrome Extension** - Uses the Zero Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Zero Chrome Extension**:
   ...
   - For Zero Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.claude/skills/\` directory.** This ensures they are automatically loaded when Zero runs in the project.
