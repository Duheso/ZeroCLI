...
function OnboardGithub(props: {
  ...
  const finalize = useCallback(
    async (
      ...
      if (!activated.ok) {
        setErrorMsg(
          `Token saved, but settings were not updated: ${activated.detail ?? 'unknown error'}. ` +
            `Add env CLAUDE_CODE_USE_GITHUB=1 and OPENAI_MODEL to your user settings.json manually.`,
        )
        setStep('error')
        return
      }
      ...
      onChangeAPIKey()
      onDone(
        'GitHub Copilot onboard complete. Copilot token and OAuth token stored in secure storage (Windows/Linux: ~/.zerocli/.credentials.json, macOS: Keychain fallback to ~/.zerocli/.credentials.json); user settings updated. Restart if the model does not switch.',
        { display: 'user' },
      )
    },
