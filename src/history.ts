...
import { getZeroConfigHomeDir, isEnvTruthy } from './utils/envUtils.js'
...
/**
 * ZeroCLI parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getZeroConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by ZeroCLI's Tungsten tool.
