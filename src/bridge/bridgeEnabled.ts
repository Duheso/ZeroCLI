 ...
 * with the claude.ai OAuth token). isZeroAISubscriber() excludes
...
// program, before enableConfigs() runs. isZeroAISubscriber() → getGlobalConfig()
...
function isZeroAISubscriber(): boolean {
  try {
    return authModule.isZeroAISubscriber()
  } catch {
    return false
  }
}
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of ZeroCLI (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`zero update\` to update.`
    }
  }
  return null
}
