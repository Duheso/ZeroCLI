 ...
 * (oauth-authed, same store), write to ~/.zerocli/uploads/{sessionId}/, and
 * return @path refs to prepend. Zero's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Zero, just without @path.
 */
...
function uploadsDir(): string {
  return join(getZeroConfigHomeDir(), 'uploads', getSessionId())
}
