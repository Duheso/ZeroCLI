...
export async function createBridgeSession({
  ...
  const { getZeroAIOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getZeroAIOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getZeroAIOAuthTokens()?.accessToken
  if (!accessToken) {
