// OAuth client for handling authentication flows with Zero services
...
/**
 * Check if the user has Zero.ai authentication scope
 * @private Only call this if you're OAuth / auth related code!
 */
export function shouldUseZeroAIAuth(scopes: string[] | undefined): boolean {
...
export function buildAuthUrl({
  ...
  port: number
  isManual: boolean
  loginWithZeroAi?: boolean
  inferenceOnly?: boolean
  orgUUID?: string
  loginHint?: string
  loginMethod?: string
}): string {
  const authUrlBase = loginWithZeroAi
    ? getOauthConfig().CLAUDE_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show Zero Max upsell
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full Zero AI set. The
  ...
  try {
    ...
    const existing = getZeroAIOAuthTokens()
