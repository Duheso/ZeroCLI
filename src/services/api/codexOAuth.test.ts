...
test('serves updated success copy after a successful Codex OAuth flow', async () => {
  ...
  ;(globalThis.fetch as any) = mock(async (input, init) => {
  ...
  expect(html).toContain('You can return to Zero CLI now.')
  expect(html).toContain(
    'Zero CLI will finish activating your new Codex OAuth login.',
  )
...
test('cancellation during token exchange returns a cancelled page and rejects the flow', async () => {
  ...
  ;(globalThis.fetch as any) = mock((input, init) => {
    const url = String(input)
    if (url.startsWith('http://localhost:')) {
      return originalFetch(input, init)
    }
  ...
  expect(html).toContain('retry in Zero CLI')
})
