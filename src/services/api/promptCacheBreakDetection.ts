...
import { getZeroTempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getZeroTempDir(), `cache-break-${suffix}.diff`)
}
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getZeroTempDir(), { recursive: true })
