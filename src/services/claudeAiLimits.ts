...
export let currentLimits: ZeroAILimits = {
  status: 'allowed',
...
type StatusChangeListener = (limits: ZeroAILimits) => void
export const statusListeners: Set<StatusChangeListener> = new Set()

export function emitStatusChange(limits: ZeroAILimits) {
...
export async function checkQuotaStatus(): Promise<void> {
  ...
  if (!shouldProcessRateLimits(isZeroAISubscriber())) {
    return
  }
 ...
 * Returns ZeroAILimits if a threshold was surpassed, null otherwise.
 */
 ...
 * Returns ZeroAILimits if thresholds are exceeded, null otherwise.
 */
...
export function extractQuotaStatusFromHeaders(
  ...
  if (!shouldProcessRateLimits(isSubscriber)) {
    ...
    if (currentLimits.status !== 'allowed' || currentLimits.resetsAt) {
      const defaultLimits: ZeroAILimits = {
        status: 'allowed',
        unifiedRateLimitFallbackAvailable: false,
        isUsingOverage: false,
      }
